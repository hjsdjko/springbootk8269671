			...
			<el-form class="center-form-pv" :style='{"padding":"20px 30px","margin":"0 0 0 30px","overflow":"hidden","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 30px)","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"0 0 10px","textAlign":"left","flexWrap":"wrap","display":"flex","width":"auto","fontSize":"inherit","order":"1"}' >
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">用户名</label>
						<el-input v-model="searchForm.nickname" placeholder="用户名" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">评论内容</label>
				...
				<el-row :style='{"margin":"0 0 10px","textAlign":"left","flexWrap":"wrap","display":"flex","width":"auto","fontSize":"inherit","order":"1"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","outline":"none","color":"#fff","borderRadius":"0 20px 20px 0","background":"#000","width":"auto","fontSize":"inherit","float":"right","transition":"all 0.3s","height":"36px"}' type="success" @click="backClick()">返回</el-button>
				</el-row>
				...
				<el-row class="actions" :style='{"margin":"0 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('discussmeishicaipin','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia7" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('discussmeishicaipin','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"calc(100% - 30px)","padding":"0 30px 30px","margin":"0px 0 0 30px","borderRadius":"0px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussmeishicaipin','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								更新
							</el-button>

							<el-button class="view" v-if="isAuth('discussmeishicaipin','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
							...
							<el-button class="btn8" type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-yanziwancheng" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								回复
							</el-button>
...
import AddOrUpdate from "./add-or-update";
	...
	this.$router.push({path:'/discussdiscussmeishicaipin',query:{refid:id}});
    },
	backClick(){
		history.back()
	},
	...
	discussmeishicaipinstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'discussmeishicaipin/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
    ...
    deleteHandler(id ,refid ) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "discussmeishicaipin/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
				this.$http({
					url: 'meishicaipin/info/' + refid,
					method: "get",
				}).then(res=>{
					if(res.data&&res.data.code==0){
						res.data.data.discussnum = res.data.data.discussnum - Number(ids.length)
						this.$http({
							url: 'meishicaipin/update',
							method: "post",
							data:res.data.data
						}).then(res1=>{
